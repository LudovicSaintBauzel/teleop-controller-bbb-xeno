...
static unsigned int pin_in = 48; // P9_15
static unsigned int pin_out = 51; // P9_16
...
//	int regval = 0;
	...
	if (rtdm_task_init(&F_sensor_read1, "Force sensor read 1", (rtdm_task_proc_t) F_sensor_read_thread, NULL, 99, (nanosecs_rel_t) READ_PERIOD) != 0)
...
static int force_sensor_close(struct rtdm_dev_context* context,rtdm_user_info_t* user_info) {
	...
	rtdm_task_destroy(&F_sensor_read1);
...
static int offset_reset(struct rtdm_dev_context *context, rtdm_user_info_t *user_info, void *buf, size_t nbyte)
{
	...
	if(temp == 59595959)
	{
		rtdm_task_destroy(&F_sensor_read1);
		force_sensor_set_offset();
		rtdm_printk(KERN_INFO "Data received from user: reset fsensor 1\n");
		if (rtdm_task_init(&F_sensor_read1, "Force sensor read 1", (rtdm_task_proc_t) F_sensor_read_thread, NULL, 99, (nanosecs_rel_t) READ_PERIOD) != 0)
			rtdm_printk(KERN_INFO "Error while creating F_sensor_read 1\r\n");		
	}
	return sizeof(int);
...
static struct rtdm_device device = {
	...
	.context_size = 0,
	.device_name = "xeno_force_sensor1",
	...
	.profile_version = 1,
	.driver_name = "xeno_force_sensor1",
	.driver_version = RTDM_DRIVER_VER(0, 0, 1),
	.peripheral_name = "rt-lkm-force_sensor1",
	.provider_name = "ISIR",
